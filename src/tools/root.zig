...
pub fn allTools(
    ...
    // Nostr tool (post, read, search, profile, react, channels)
    {
        const nt = try allocator.create(nostr.NostrTool);
        nt.* = .{ .config_dir = workspace_dir, .allocator = allocator };
        try list.append(allocator, nt.tool());
    }
