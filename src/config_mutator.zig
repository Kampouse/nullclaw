...
fn ensureObject(value: *std.json.Value, _: std.mem.Allocator) *std.json.ObjectMap {
    switch (value.*) {
...
fn setAtPath(
    ...
    for (tokens[0 .. tokens.len - 1]) |token| {
        ...
        try obj.put(allocator, key_copy, .{ .object = std.json.ObjectMap.empty });
...
pub fn getPathValueJson(allocator: std.mem.Allocator, path: []const u8) ![]u8 {
    ...
    if (root != .object) {
        root = .{ .object = std.json.ObjectMap.empty };
    }
