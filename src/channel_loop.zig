...
pub const ChannelRuntime = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: *const Config, io: std.Io) !*ChannelRuntime {
        ...
        const tools = tools_mod.allTools(allocator, config.workspace_dir, io, .{
            ...
            .predict_provider = config.predict.provider,
            .predict_model = config.predict.model,
            .predict_base_url = config.predict.base_url,
            .predict_tools = config.predict.tools,
            .predict_max_rounds = config.predict.max_rounds,
